    ...
    public void beforeTestStop(TestResult result) {
        if (result.getStatus() == Status.FAILED || result.getStatus() == Status.SKIPPED || result.getStatus() == Status.BROKEN) {
            if (DriverManager.getDriver() != null) {
                ...
                Allure.addAttachment(result.getName() + "_Failed_Screenshot", new ByteArrayInputStream(((TakesScreenshot)((SelfHealingDriver) DriverManager.getDriver()).getDelegate()).getScreenshotAs(OutputType.BYTES)));
            }
